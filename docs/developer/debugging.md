...
Tracy is a real-time profiler integrated into Nomad for performance analysis.

### Enabling Tracy

**Option 1: CMake Configure**
```bash
cmake -S . -B build -DNOMAD_PROFILE_ENABLED=ON
cmake --build build
```

**Option 2: Code Define**
```cpp
// In NomadCore/NomadProfiler.h
...
1. **Instrument Code:**
   ```cpp
   #include "NomadCore/NomadProfiler.h"
   
   void MyFunction() {
       NOMAD_ZONE("MyFunction");  // Start profiling zone
       
       // Your code here
       
   }  // Zone ends automatically
   ```

2. **Launch Tracy Server:**
   - Download from [Tracy Releases](https://github.com/wolfpld/tracy/releases)
   - Run `Tracy.exe`

3. **Run Nomad:**
   - Launch Nomad (with Tracy enabled)
   - Tracy automatically connects

4. **Analyze Performance:**
   - View timeline of function calls
   - Identify bottlenecks
   - Measure frame times

### Example: Profiling Audio Processing

```cpp
void TrackManager::processAudio(float* buffer, uint32_t frames, double time) {
    NOMAD_ZONE("TrackManager::processAudio");
    
    {
        NOMAD_ZONE("Clear Buffer");
        memset(buffer, 0, frames * 2 * sizeof(float));
    }
    
    for (auto& track : m_tracks) {
        NOMAD_ZONE("Process Track");
        track->processAudio(buffer, frames, time);
    }
    
    {
        NOMAD_ZONE("Apply Master Volume");
...
processAudio ——————————————————— 2.3ms
├─ Clear Buffer ———————————————— 0.1ms
├─ Process Track (Track 1) ———— 0.8ms
├─ Process Track (Track 2) ———— 0.9ms
└─ Apply Master Volume ————————— 0.5ms
```

### Tracy HUD Overlay

Press `F12` to toggle built-in FPS display:
- Current FPS
- Frame time (ms)
- Target FPS
- Mode (Adaptive/Fixed)
